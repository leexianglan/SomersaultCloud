...
- (instancetype)initWithFrame:(CGRect)frame texts:(NSArray *)textsArr colorArr:(NSArray *)colorArr
{
...
- (void)startCycling
{
    ...
    if ([_textsArr[_index] isKindOfClass:[NSString class]]) {
        [UIView animateWithDuration:0.5 animations:^{ _label.text = _textsArr[_index]; }];
    }else if ([_textsArr[_index] isKindOfClass:[NSArray class]]){
        [UIView animateWithDuration:0.5 animations:^{
            [[LXViewControllerManager shareManager] setMutableAttributedStringLbl:_label strArr:_textsArr[_index] colorArr:self.colorArr];
        }];
    }

...
- (instancetype)initWithFrame:(CGRect)frame andStyleModel:(QHNavSliderMenuStyleModel *)aStyleModel andDelegate:(id<QHNavSliderMenuDelegate>)delegate  showType:(QHNavSliderMenuType)type;{
    if (self=[super initWithFrame:frame]) {
        ...
        [self addBottomLineWithFrame:frame];
    }
    return self;
}

-(void)addBottomLineWithFrame:(CGRect)frame{
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(frame) - 0.3, CGRectGetWidth(frame), 0.3)];
    view.backgroundColor = [UIColor colorWithRed:217/255.f green:217/255.f blue:217/ 255.f alpha:1];
...
- (void)adjustToScrollView :(NSInteger)row {
    ...
    if(styleModel.sizeToFitScreenWidth) {
        ...
        float offsetx = selectBtn.left - screenWidth/2.f+ styleModel.menuWidth/2.f;
        
        offsetx=offsetx<0?0:offsetx;
        offsetx = offsetx+screenWidth>(contentScrollView.contentSize.width)?contentScrollView.contentSize.width-screenWidth:offsetx;
...
- (void)initSliderTopNavView:(QHNavSliderMenuType)type {
    ...
    if (styleModel.menuWidth <=0) {
        styleModel.menuWidth = screenWidth /2.f;
    }
...
//        CGRect rect = CGRectMake((styleModel.menuHorizontalSpacing+styleModel.menuWidth)*i, 0, styleModel.menuWidth, CGRectGetHeight(contentScrollView.bounds));
        UIButton *btn =[[UIButton alloc] initWithFrame:CGRectMake((styleModel.menuHorizontalSpacing+styleModel.menuWidth)*i, 0, styleModel.menuWidth, CGRectGetHeight(contentScrollView.bounds))];
        btn.centerY = CGRectGetHeight(contentScrollView.bounds)/2.f;
        ...
        if(styleModel.sizeToFitScreenWidth) {
            ...
            btn.centerX = screenWidth /2.f-((int)(styleModel.menuTitles.count/2)-i)*leftGap;
        }
        ...
        if (styleModel.sizeInMiddle) {
            ...
            btn.centerX = screenWidth/2.f + (float)(i-(styleModel.menuTitles.count - 1.f)/2.f)*(styleModel.menuWidth + styleModel.menuHorizontalSpacing/2.f);
        }
    ...
    if (menuType==QHNavSliderMenuTypeTitleOnly) {
        ...
        bottomTabLine=[[UIView alloc] initWithFrame:CGRectMake(styleModel.menuHorizontalSpacing+styleModel.menuWidth/4.f, 0, styleModel.menuWidth/2.f, styleModel.lineHeight>0?styleModel.lineHeight:2)];
        bottomTabLine.bottom = CGRectGetHeight(contentScrollView.bounds);

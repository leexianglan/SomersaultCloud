...
+ (YBPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles menuWidth:(CGFloat)itemWidth delegate:(id<YBPopupMenuDelegate>)delegate tag:(NSInteger)tag
{
    YBPopupMenu *popupMenu = [[YBPopupMenu alloc] init];
    popupMenu.tag = tag;
    popupMenu.point = point;
...
//    }];
    if (self.dismissNotify) {
        [[LXNotifyManager shareInstance] postNotifyKey:kkkHideYBPopView userInfo:@{@"tag":[NSString stringWithFormat:@"%ld", self.tag]}];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell =  [[[NSBundle mainBundle]loadNibNamed:@"YBPopupMenuCell" owner:nil options:nil] lastObject];
//        [[YBPopupMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
//        cell.textLabel.numberOfLines = 0;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    cell.backgroundColor = [UIColor clearColor];
    cell.textLabel.textColor = [UIColor whiteColor];
//    cell.lbl.textColor = _textColor;
    cell.textLabel.font = [UIFont systemFontOfSize:_fontSize];
    NSString *dic = _titles[indexPath.row];
    NSString *name = dic;
//    if ([_titles[indexPath.row] isKindOfClass:[NSAttributedString class]]) {
//        cell.lbl.attributedText = _titles[indexPath.row];
//    }else if ([_titles[indexPath.row] isKindOfClass:[NSString class]]) {
//        cell.lbl.text = _titles[indexPath.row];
//    }else {
//    }
    cell.line.hidden = NO;
    if ([dic isEqual:[_titles lastObject]]) {
...
//    _borderColor = [UIColor lightGrayColor];
    _arrowWidth = 15.0;
    _arrowHeight = 10.0;
//    _backColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    self.arrowDirection = YBPopupMenuArrowDirectionTop;
    ...
    _menuBackView.backgroundColor = [UIColor clearColor];// [[UIColor blackColor] colorWithAlphaComponent:0.1];
//    _menuBackView.alpha = 0;
...
- (void)updateUI
{
    ...
    if (_point.y - height - _arrowHeight  - _offset< 87 && _priorityDirection == YBPopupMenuPriorityDirectionBottom) {
        _priorityDirection = YBPopupMenuPriorityDirectionTop;
        [self updateUI];
        return;
    }
...
//     lxl
//    if (!self.imgView) {
//        self.imgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"help_more_bg"]];
//        [self addSubview:self.imgView];
//        [self bringSubviewToFront:self.tableView];
//    }
    self.imgView.frame = CGRectMake(0, 0, CGRectGetWidth(frame), CGRectGetHeight(frame));
}
